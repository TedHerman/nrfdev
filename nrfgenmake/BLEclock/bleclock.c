...
static uint8_t twistate;  // used to control type of I2C interaction
  enum { // what can be in twistate
    twi_clock_read = 0, 
    twi_clock_write = 1,
    twi_eeprom_read = 2,
    twi_eeprom_write = 3,
    };  
static bool twirxdone;     // set to true by a read operation finishing
static uint32_t lasttime;  // read/written to/from eeprom, as number of seconds
...
uint8_t twi_buffer[10];  // adjust this to be maximum size for all purposes
...
void clockread_handler(nrf_drv_twi_evt_t const * p_event, void * p_context);
void clockwrite_handler(nrf_drv_twi_evt_t const * p_event, void * p_context);
void eepromread_handler(nrf_drv_twi_evt_t const * p_event, void * p_context);
void eepromwrite_handler(nrf_drv_twi_evt_t const * p_event, void * p_context);
void twi_handler(nrf_drv_twi_evt_t const * p_event, void * p_context) {
  gpioOn(22);
  switch (twistate) {
    case twi_clock_read: 
      clockread_handler(p_event,p_context);
      break;
    case twi_clock_write: 
      clockwrite_handler(p_event,p_context);
      break;
    case twi_eeprom_read:
      eepromread_handler(p_event,p_context);
      break;
    case twi_eeprom_write:
      eepromwrite_handler(p_event,p_context);
      break;
    default:
      break;
    } 
  }
// function for handling twi events for clock reading
void clockread_handler(nrf_drv_twi_evt_t const * p_event, void * p_context) {
  DS3231clock_t * t;
  switch (p_event->type) {
...
void DS3231_read_curtime() {
  ...
  twistate = twi_clock_read; 
  nrf_drv_twi_tx(&m_twi_port,0xD0>>1,twi_buffer,1,true);
  }

// function for handling twi events for clock writing 
void clockwrite_handler(nrf_drv_twi_evt_t const * p_event, void * p_context) {
  // nothing to do here!
  }
void DS3231_set_curtime(clock_read_t * p) {
  // p is pointer to a "clock_read_t" struct
  // which has month, day, hour, and minute 
  DS3231clock_t * t;  // what will be written to DS3231 chip 
  t = (DS3231clock_t *) &twi_buffer[1];
  t->month = ((p->month/10)<<4) | ((p->month%10)&0xf);
  t->day   = ((p->day/10)<<4) | ((p->day%10)&0xf);
  t->hour  = ((p->hour/10)<<4) | ((p->hour%10)&0xf);
  t->minute = ((p->minute/10)<<4) | ((p->minute%10)&0xf);
  t->second = 0; // not that precise of a setting
  t->weekday = 1;  // we just don't care what day of week
  t->year = 0;  // pretend it's 2000 NOTE FOR FUTURE MAYBE WE CAN DO BETTER
  twi_buffer[0] = 0;   // register address on DS3231 chip
  twistate = twi_clock_write; 
  nrf_drv_twi_tx(&m_twi_port,0xD0>>1,twi_buffer,1+sizeof(DS3231clock_t),true);
  }

// function for handling twi events for eeprom reading
void eepromread_handler(nrf_drv_twi_evt_t const * p_event, void * p_context) {
  switch (p_event->type) {
    case NRF_DRV_TWI_RX_DONE:
      memcpy(&lasttime,twi_buffer,4);  // copy laststime bytes from I2C buffer
      twirxdone = true;
      break;
    case NRF_DRV_TWI_TX_DONE:
      nrf_drv_twi_rx(&m_twi_port,0xAE>>1,twi_buffer,4,false);
      break;
    default:
      break;
    }
  }
// function to read eeprom at location 0 into lasttime variable
void eepromread() {
  twi_buffer[0] = twi_buffer[1] = 0;   // register address on AT24C32 chip
  twistate = twi_eeprom_read;
  nrf_drv_twi_tx(&m_twi_port,0xAE>>1,twi_buffer,2,true);
  }

// function for handling twi events for eeprom writing 
void eepromwrite_handler(nrf_drv_twi_evt_t const * p_event, void * p_context) {
  // nothing to be done here
  }
// function to write eeprom at location 0 from the lasttime variable
void eepromwrite() {
  twi_buffer[0] = twi_buffer[1] = 0;   // register address on AT24C32 chip
  memcpy(&twi_buffer[2],&lasttime,4);  // copy value from lastttime
  twistate = twi_eeprom_write;
  nrf_drv_twi_tx(&m_twi_port,0xAE>>1,twi_buffer,6,true);
  }

// function to set lasttime variable from the current clock
void set_lasttime() {
  lasttime = clock.minute * 60;  // initial seconds 
  lasttime += clock.hour * 60 * 60;  
  lasttime += clock.day * 24 * 60 * 60;  
  lasttime += clock.month * 31 * 24 * 60 * 60;
  }
...
static void sleep_mode_enter(void) {
  //sd_power_ramon_set(0x0); // when set to 0x0, no RAM areas are to be saved while off
...
int main(void) {
  init_all();
  nrf_delay_ms(10);   // wait for a bit after first power on
  connected = false;        
...
static void gpioteHandler(nrf_drv_gpiote_pin_t p, nrf_gpiote_polarity_t d) {
  ...
  nrf_drv_gpiote_in_event_disable(16);									//TIEN CHANGES: commented line out
  //gpioToggle(22);  // DEBUG 
  if (p != 16) return; // Pin 16 is the "KEY1" button on the breakout board
  
  // this code is supposed to prevent button from being activated too frequently,
  // but right now it might be too crude (just waits for a minute)
  twirxdone = false;
  gpioOff(22);
  DS3231_read_curtime();    // get timestamp of this button press

  while (!twirxdone) { 
    // WARNING potential infinite loop if I2C not working properly
    nrf_delay_ms(1);	
    }
  
  twirxdone = false;
  eepromread();   // read old value into lasttime 
  while (!twirxdone) { 
    // WARNING potential infinite loop if I2C not working properly
    nrf_delay_ms(1);
    }
  ...
  oldevent = lasttime;
  set_lasttime();  // set new lasttime from clock
  if (lasttime == oldevent) return;  // same minute as last time
  eepromwrite();

  sd_softdevice_disable();  // force restart of softdevice, even if it was active
  init_all(); 
  nrf_drv_gpiote_in_event_disable(16);  // because init_all() re-enabled pin 16
  connected = false;      
  ble_advertising_start(BLE_ADV_MODE_FAST);  // start sending beacons
  }
