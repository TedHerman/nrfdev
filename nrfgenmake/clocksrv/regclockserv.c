...
void blesetclock(void) {
  ...
  err_code = sd_ble_gatts_value_set(
	m_notif.our_service.conn_handle,
...
static void updateclock(void) {
  if (m_notif.our_service.conn_handle == BLE_CONN_HANDLE_INVALID) return;
  blesetclock();
  /*
  uint16_t len = 4;
  if (m_notif.our_service.conn_handle == BLE_CONN_HANDLE_INVALID) return;
  ...
  hvx_params.handle = m_notif.our_service.char_handles[CLOCK_READ_INDX].value_handle;
  hvx_params.type = BLE_GATT_HVX_NOTIFICATION;
  hvx_params.offset = 0;
  hvx_params.p_len = &len;
  hvx_params.p_data = (uint8_t*)&m_notif.clock;
  sd_ble_gatts_hvx(m_notif.our_service.conn_handle,&hvx_params);
  */
  }
...
void ble_notif_on_gatt_evt(notif_t * p_notif, nrf_ble_gatt_evt_t const * p_gatt_evt) {
  if ((p_notif->our_service.conn_handle == p_gatt_evt->conn_handle)
...
static void charac_add(
  ...
  err_code = characteristic_add(m_notif.our_service.service_handle,
		  p_params,
		  p_handles);
  APP_ERROR_CHECK(err_code);
  }
...
static void local_service_add() {
  ...
  m_notif.our_service.conn_handle = BLE_CONN_HANDLE_INVALID;   
  ...
  memset(&m_notif.our_service, 0, sizeof(m_notif.our_service));
  m_notif.our_service.conn_handle = BLE_CONN_HANDLE_INVALID;
  err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, 
		 &m_notif.service_uuid, 
		 &m_notif.our_service.service_handle);
  // presumably that assigned into the service_handle
  APP_ERROR_CHECK(err_code);
  }

/*
static void Xlocal_service() {
  ret_code_t err_code;
  ble_uuid_t ble_uuid;
...
static void on_conn_params_evt(ble_conn_params_evt_t * p_evt) {
  if (p_evt->evt_type == BLE_CONN_PARAMS_EVT_FAILED) {
    sd_ble_gap_disconnect(m_notif.our_service.conn_handle, BLE_HCI_CONN_INTERVAL_UNACCEPTABLE);
    }
  }
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context) {
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            APP_ERROR_CHECK(err_code);
            m_notif.our_service.conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
...
void bsp_event_handler(bsp_event_t event) {
    ...
    switch (event) {
        ...
        case BSP_EVENT_DISCONNECT:
            err_code = sd_ble_gap_disconnect(m_notif.our_service.conn_handle,
                          BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
        ...
        case BSP_EVENT_WHITELIST_OFF:
            if (m_notif.our_service.conn_handle == BLE_CONN_HANDLE_INVALID) {
