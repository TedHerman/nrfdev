...
void flipled(void * parameter, uint16_t size) {
    ...
    err_code = app_timer_start(blink_timer_id,
               APP_TIMER_TICKS(3000+randvalue%501), NULL);
    APP_ERROR_CHECK(err_code);
    err_code = app_uart_put('\n');
    NRF_LOG_INFO("put newline %d",err_code);
    err_code = app_uart_put('>');
    NRF_LOG_INFO("put > %d",err_code);
    err_code = app_uart_put(' ');
    NRF_LOG_INFO("put > %d",err_code);
    err_code = app_uart_get(&character);
    NRF_LOG_INFO("get %d",err_code);
    if (err_code == NRF_SUCCESS) {
       err_code = app_uart_put(character);
       }
    }
