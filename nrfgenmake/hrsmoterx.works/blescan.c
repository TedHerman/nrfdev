...
APP_TIMER_DEF(restart_timer_id);
static void restart_timeout_handler(void * p_context);
void motesetup(void);
...
static void bas_c_evt_handler(ble_bas_c_t * p_bas_c, ble_bas_c_evt_t * p_bas_c_evt)
{
    ...
    switch (p_bas_c_evt->evt_type)
    {
	    ...
	    NRF_LOG_INFO("progmode = %d",progmode);
	    if (progmode == 0) {
	    progmode = 1;  // stop new scans
            nrf_ble_scan_stop();
            err_code = app_timer_create(&restart_timer_id,
                         APP_TIMER_MODE_SINGLE_SHOT,
                         restart_timeout_handler);
            APP_ERROR_CHECK(err_code);
            err_code = app_timer_start(restart_timer_id,
...
static void scan_evt_handler(scan_evt_t const * p_scan_evt)
{
    ...
    switch(p_scan_evt->scan_evt_id)
    {
	    ...
	    NRF_LOG_INFO("Connecting error %d",err_code);
            APP_ERROR_CHECK(err_code);
        } break;
...
void app_error_handler(uint32_t error_code, 
		uint32_t line_num, const uint8_t * p_file_name)
{ nnq_err = error_code;
  nnq_line =  line_num;
  memcpy(nnq_file,p_file_name,64); 
  NRF_LOG_INFO("App crash %s line %d error code %d.",nnq_file,nnq_line,nnq_err);
  NRF_LOG_FLUSH();
  while (1) { };
}

void restartproc(void * parameter, uint16_t size) {
    if (nrf_sdh_is_enabled()) {
       NRF_LOG_INFO("attempt sdh disable");	    
       bsp_indication_set(BSP_INDICATE_IDLE);
       // nrf_sdh_disable_request();
       disreq = true;
       return;
       }
    NRF_LOG_INFO("restart/enable");
    NRF_LOG_FLUSH();
    progmode = 2;
    ble_stack_init();
    motesetup();
...
static void restart_timeout_handler(void * p_context) {
    app_sched_event_put(NULL,1,&restartproc);
    }
