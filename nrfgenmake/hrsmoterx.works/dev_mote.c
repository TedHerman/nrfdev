...
void moterx(void * parameter, uint16_t size) {
    uint8_t c;
    ret_code_t err_code;
    // uint32_t ticksremain = nrf_802154_rsch_timeslot_us_left_get();

    NRF_LOG_INFO("hfclk_is_ready = %d",
		    nrf_802154_clock_hfclk_is_running());

    if (blestack) {
       NRF_LOG_INFO("turning off ble stack");
       nrf_ble_scan_stop();
    ...
    NRF_LOG_INFO("slot state = %d microseconds remaining = %d",
		    nnqslot,ticksremain);
    if (nnqslot == 0) nrf_802154_rsch_timeslot_request(200*1000);
    if (needinit) {
       NRF_LOG_INFO("motesetup again");
       motesetup();
       err_code = app_timer_start(mote_timer_id, 
		    APP_TIMER_TICKS(100), NULL);
       APP_ERROR_CHECK(err_code);
    ...
    NRF_LOG_INFO("802154 state needs reset");
    nrf_802154_sleep();
    nrf_802154_deinit();
    needinit = true;
    err_code = app_timer_start(mote_timer_id, 
