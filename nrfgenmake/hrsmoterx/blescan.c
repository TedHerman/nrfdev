...
bool ble_communicated = false;
bool moteswitch = false;
...
int main(void)
{
    ...
    {
	app_sched_execute();
	if (!ble_communicated) idle_state_handle();
	else {  // ble communicated
	   if (!moteswitch) {
              nrf_ble_scan_stop();
              err_code = bsp_indication_set(BSP_INDICATE_IDLE);
              APP_ERROR_CHECK(err_code);
