...
void flipled(void * parameter, uint16_t size) {
    ...
    uint8_t rgb1[3] = {0xff,0x00,0xff};
    uint8_t rgb2[3] = {0x00,0xff,0x00};
    NRF_LOG_INFO("blink %d",counter++);
    bsp_board_led_invert(0);
    if (counter % 2 == 0) neopixel_write(rgb1);
    else neopixel_write(rgb2);
    err_code = nrf_drv_rng_rand(p,4);
    APP_ERROR_CHECK(err_code);
    err_code = app_timer_start(blink_timer_id,
               APP_TIMER_TICKS(500+randvalue%501), NULL);
    APP_ERROR_CHECK(err_code);
    }
...
int main(void) {
	...
	app_sched_execute();
	// NRF_LOG_PROCESS() 
	// see https://infocenter.nordicsemi.com/index.jsp?topic=%2Fcom.nordic.infocenter.sdk5.v14.0.0%2Fgroup__nrf__log__ctrl.html
	NRF_LOG_FLUSH();
        nrf_pwr_mgmt_run();
        }
    }
