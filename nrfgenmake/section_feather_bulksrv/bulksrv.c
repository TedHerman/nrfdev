...
#define MEMCHUNKS_POOL 720 // total memory size is around 170kB 
...
// put memchunk area into a data section (see s140.ld)
typedef struct {
    uint8_t prefence[4];  // will be set to "fnce"
    uint16_t memchunk_index;  // range is 0 to MEMCHUNKS_POOL
    uint8_t memchunk_area[DATA_LENGTH_MAX*MEMCHUNKS_POOL];
...
static void mem_init() {
  ...
  char * fence = "fnce";
  NRF_LOG_INFO("memchunks initialized");
  ...
  for (int i=0; i<MEMCHUNKS_POOL; i++) { 
     ...
     memset(p_memchunks[i],0,DATA_LENGTH_MAX);
     }
  }
...
void transfer_data(void * parameter, uint16_t size) {
  ...
  // printf("total transfer time %d seconds\n",(int)m_bulksrv.time);
  app_timer_stop_all();
...
void readstart(void * parameter, uint16_t size) {
  ...
  if (m_bulksrv.notificationset) {
      ...
      NRF_LOG_INFO("time %d receive %c .. %c surplus %d count %d",
		      ...
		      m_bulksrv.bulkcount);
      */
      NRF_LOG_INFO("time %d received count = %d",
	m_bulksrv.time,memchunk_p.memchunk_index);
      }
...
void on_hvx(ble_evt_t const * p_ble_evt) {
  ...
  if (p_ble_evt->evt.gattc_evt.params.hvx.handle == m_bulksrv.clockread_handle) {
    ...
    memcpy(p_memchunks[memchunk_p.memchunk_index++],
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context) {
    ...
    if (memchunk_p.memchunk_index >= MEMCHUNKS_POOL && !m_bulksrv.stopped) {
        sd_ble_gap_adv_stop(m_adv_handle);
	m_bulksrv.stopped = true;
        err_code = sd_ble_gap_disconnect(p_ble_evt->evt.gattc_evt.conn_handle,
                                     BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
